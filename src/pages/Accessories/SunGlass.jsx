...
const SunGlass = () => {
  ...
  const defaultGlasses = [
    { title: "Classic Aviator", price: 4999, rating: 4.8, image: Glass1 },
    { title: "Retro Round Glasses", price: 3999, rating: 4.6, image: Glass2 },
    { title: "Sporty Sunglasses", price: 5999, rating: 4.7, image: Glass3 },
  ];
    ...
    const fetchGlasses = async () => {
      try {
        ...
        if (res.data.success && res.data.data.length > 0) {
          ...
          setGlasses(defaultGlasses);
        }
      } catch (err) {
        console.error("Failed to fetch sunglasses", err);
        setGlasses(defaultGlasses);
      }
    };
        ...
        {glasses.map((sunGlass, index) => (
          ...
          >
            <div className="relative">
              <img
                src={
                  sunGlass.image?.startsWith("http") || sunGlass.image?.startsWith("/")
                    ? sunGlass.image
                    : `http://localhost/fashion_backend/uploads/${sunGlass.image}`
                }
                alt={sunGlass.title}
