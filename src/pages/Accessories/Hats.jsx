...
const Hats = () => {
  ...
  const defaultHats = [
    { title: "Classic Fedora Hat", price: 3999, rating: 4.5, image: Hat1 },
    { title: "Beach Sun Hat", price: 2999, rating: 4.6, image: Hat2 },
    { title: "Casual Baseball Cap", price: 1999, rating: 4.4, image: Hat3 },
  ];
    ...
    const fetchHats = async () => {
      try {
        ...
        if (res.data.success && res.data.data.length > 0) {
          ...
          setHats(defaultHats); // fallback
        }
      } catch (err) {
        console.error("Failed to fetch hats", err);
        setHats(defaultHats);
      }
    };
  ...
  return (
        ...
        {hats.map((hats, index) => (
          ...
          >
            <div className="relative">
              <img
                src={
                  hats.image?.startsWith("http") || hats.image?.startsWith("/")
                    ? hats.image
                    : `http://localhost/fashion_backend/uploads/${hats.image}`
                }
                alt={hats.title}
    ...
    //   >
    //     <div className="max-w-7xl mx-auto px-6 text-center">
    //       <h1 className="text-4xl font-bold text-yellow-400">Hats Collection</h1>
    ...
    //       >
    //         <div className="relative">
    //           <img src={hast.img} alt={hats.title} className="w-full h-64 object-cover" />
    //           <button
    //             className={`absolute top-3 right-3 p-2 rounded-full shadow transition ${
    //           likedIndex === index
    //             ? "bg-yellow-600"
    ...
    //             <FiHeart
    //               className={`text-lg ${
    //                  likedIndex === index ? "text-white" : "text-yellow-400"}`}
    //             />
    //           </button>
    //         </div>
    //         <div className="p-4">
    ...
    //   >
    //     <div className="max-w-7xl mx-auto px-6 text-center">
    //       <h1 className="text-4xl font-bold text-teal-400">Hats Collection</h1>
